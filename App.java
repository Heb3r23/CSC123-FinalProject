		...
		while (menuSelection != 8) {
			if (menuSelection == 1) {
				...
				if(p.isAdult()){
					...
					if(!g.isAdult()) {
						System.out.println("Error, entered guardian is also a child. Membership not created, please try again.");
					}
					else {
				...
				type = input.nextLine();
				System.out.println("Please enter replacement value (enter amount of dollars in numbers not words)");
				...
				if (type.equalsIgnoreCase("Book")) {
					...
					System.out.println("Please enter the number of pages of the book (enter numbers not words): ");
